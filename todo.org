...
- [ ] when migrations caused errors, then when anchovy is run again it should
      stop immediately.
- [ ] when migrations caused errors, there should be a command to mark them as
      fixed, from where we can continue running anchovy
- [ ] robustness: use 'set -e', and other good shell scripting practices (see
      http://mywiki.wooledge.org/BashFAQ, books, ..)
- [ ] support other databases: sqlite, postgres, ..?
- [ ] think about: what to do when there is already a migrations table, but
      it's not created by anchovy?
- [ ] think about: should we really require the database password to be in the
      migrations configuration file? maybe this can be solved by supplying
      config parameters via env vars
...
- [ ] think about: should we really manage one database at a time? or should
      we anchovy 'sets of databases'? yagni for now
